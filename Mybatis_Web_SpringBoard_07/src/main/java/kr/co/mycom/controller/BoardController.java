...
@Controller
public class BoardController { // 보드컨트롤러는 SqlSession에 의존한다
	...
	public String write(BoardDTO dto) {
		...
		// BoardDAO.class : 리플렉터 -> DAO가 갖고있는 함수들을 session에 넘겨준다
		BoardDAO boardDao = sqlsession.getMapper(BoardDAO.class);
		//**************************************
		// mapper xml 파일과 DAO interface 동기화
		// 인터페이스가 가지는 자원은 실행 : mapper 가 실행
		// 전제 : 1. namespace , 2. id=method
		// 1. mapper의 namespace와 DAO가 가지는 interface 이름과 동일하게 
		// 2. mapper의 함수 id를 interface의 메소드 이름과 동일하게
	...
	public String list(Model model, String pg) {
		...
		map.put("start", start);
		map.put("end", end);
		// MyBatis에 DTO를 result Param으로 주고 resultType으로 DTO를 받으면
		// 10건의 DTO데이터를 10개 만들고 list 만든 후 주소를 리턴한다 >> 내부적으로 ArrayList add
